...
const testArray = ['4','7','9','3','1']
//Given 1000 digits of PI as strings, return an array of the digits as numbers
const stringsToNum = (numbers) => {
    let numArr = numbers.map(Number)
    return numArr
}

console.log(stringsToNum(strNums));
console.log(stringsToNum(testArray));
...
console.log(sumEvens(testArray));



//Find the index of the first value when added to its index = 512
const atxIdx = (arr) => {
    let index;
    let value;
    let counter = 0;
    for(let i=0; i< arr.length; i++){
        ...
        if(index+value==512){
            console.log(`index: ${index}, value: ${value}`) // index: 505, value: 7
            return;
        }
    }
};

console.log(atxIdx(strNums)); //index 505, value 7
console.log(atxIdx(testArray)); // undefined because it wont exist in this small created array
...
const weather = [
  ...
  //will pick out the weather state of all 4 objects in the array
  const weatherStates = weather.map(weatherState => weatherState['weather_state_name'])
    //filter out all duplicates in the new created array
    .filter((weatherState, index, newArray) => newArray.indexOf(weatherState) === index);
  console.log(weatherStates)

  //find the id of the object in weather that has a min_temp of 15.915

  const idealTemp = weather.find(weatherState => weatherState['min_temp'] === 15.915)['id'];
